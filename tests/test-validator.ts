...
function extractSetFromSource(filePath: string, varName: string): Set<string> {
  ...
  const re = new RegExp("(?:export\\s+)?const " + varName + "\\s*=\\s*new Set\\(\\[([\\s\\S]*?)\\]\\)", "m");
...
// v0.8.1: All blocklists now live in constants.ts (canonical source)
const constantsFile = path.resolve(testDir, "..", "src", "pattern", "constants.ts");

// Extract canonical blocklists from the single source of truth
const ACEFORGE_TOOL_BLOCKLIST = extractSetFromSource(constantsFile, "ACEFORGE_TOOL_BLOCKLIST");
const ACEFORGE_CAPTURE_BLOCKLIST = extractSetFromSource(constantsFile, "CAPTURE_BLOCKLIST");
const SELF_TOOLS = extractSetFromSource(constantsFile, "SELF_TOOLS");
const NATIVE_TOOLS_SET = extractSetFromSource(constantsFile, "NATIVE_TOOLS");

// For backward-compatible test assertions, alias the canonical blocklist
const CROSS_SESSION_BLOCKLIST = ACEFORGE_TOOL_BLOCKLIST;
const GAP_DETECT_BLOCKLIST = ACEFORGE_TOOL_BLOCKLIST;

// File paths for drift detection (verify they DON'T define their own blocklists)
